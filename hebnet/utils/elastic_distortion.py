"""this file was taken from https://github.com/vsvinayak/mnist-helper"""
import cv2
import numpy
import math
...
def deskew(image, image_shape, negated=False):
    ...
    :param negated: a boolean flag telling  whether the input
      image is a negated one
...
def resize_img(image, target_shape, value=255, min_padding=2, negated=False):
    ...
    # Add padding
    # The aim is to make an image of different width and height
    # to a sqaure image For that first the biggest attribute among width
    # and height are determined.
    max_index = numpy.argmax([image_height, image_width])

    # if height is the biggest one, then add padding to width until
    # width becomes equal to height
    if max_index == 0:
        padded_img = cv2.copyMakeBorder(
            image, min_padding, min_padding,
            (image_height + min_padding - image_width) / 2,
            (image_height + min_padding - image_width) / 2,
            cv2.BORDER_CONSTANT, value=value
        )

    # else if width is the biggest one, then add padding to
    # height until height becomes equal to width
    else:
        padded_img = cv2.copyMakeBorder(
            image,
...
def elastic_transform(image, kernel_dim=13, sigma=6, alpha=36, negated=False):
    ...
    displacement_field_x = random_integers(-1, 1, size=image.shape) * alpha
    displacement_field_y = random_integers(-1, 1, size=image.shape) * alpha
    ...
    # make the distortrd image by averaging each pixel value
    # to the neighbouring four pixels based on displacement fields
...
def width_normalization(image, width, target_shape, negated=False):
    ...
    if width == -1:
        ...
        width_normalized_img = cv2.resize(cropped_img,
...
if __name__ == '__main__':
    ...
    from hebnet import pad_image

    fname = sys.argv[1]
    img = imread(fname, flatten=True)
    img = pad_image(img)
    imshow(img)
    dist_img = elastic_transform(img, kernel_dim=23, sigma=2., alpha=2.,
                                 negated=True)
    imshow(dist_img)
